  ...
  // list to wade through, and the opening should look like something instead. All were
  // authored in the app — Fetingen and Round and round lifted from the published profiles
  // they live in (dyze and Erbsman, `collection` stripped so a new visitor sees them as
  // their own scenes rather than somebody else's collection), Julia shapes made for the
  // library itself.
  // Blank canvas rides the REAL library only, APPENDED never prepended — the first visit
  // applies index 0 and the opening frame should be a finished picture rather than an empty
  // one. The perEffectPresets fallback doesn't take it: every scene there already IS the
  // neutral defaults, one of them already Plasma's.
  function defaultPresets() {
    ...
    return lib.length ? lib.concat(blankPreset() || []) : perEffectPresets();
  }
  ...
  function perEffectPresets() {
    return EFFECTS.map((f, e) => neutralPreset(e, f.presetName || f.name));
  }
  // THE one literal for "a scene at the shipped defaults". perEffectPresets and blankPreset
  // both build from it, so the next per-effect map added to the preset shape (the way btune
  // arrived) cannot be added to one and silently missed in the other.
  function neutralPreset(e, name) {
    return { name, effect: e, state: presetState(e), beat: presetBeat(e), pulse: presetPulse(e), plen: presetPlen(e), btune: presetBtune(), extra: presetExtra(e), beatTune: mergeBeatTune(null), ranges: {} };
  }
  ...
  //
  // Built from the shipped defaults, NOT hand-authored into DEFAULT_LIBRARY: "the defaults" is
  // precisely what this scene is meant to be, and a wire-format blob would freeze today's answer
  // and drift from it. Plasma because presetFilters(e) is EFFECTS[e].filters || [] and only
  // Fractal flames and Boids declare one, because every filter reads clearly over it, and because
  // effect defaults are neutral — so it sits STILL, and pulling a slider's two thumbs apart is a
  // visible before-and-after. That stillness is also why it must NOT ride the auto-cycle show
  // (inRotation: absent means IN), hence rotate: false — the scene list's checkbox opts it back
  // in, and autosavePreset carries the flag, so work built on it stays out until ticked. No
  // `layers` key, so it is a single-item stack by construction. A retired plasma id returns null
  // and the caller drops the scene — the wire format's rule, unknown id ⇒ dropped, never misfiled
  // onto whatever effect happens to sit at index 0.
  function blankPreset() {
    const e = effectIndexFromId("plasma");
    if (e < 0) return null;
    const p = neutralPreset(e, "Blank canvas");
    p.rotate = false;
    return p;
  }
  ...
  // WHOSE collections you are holding — `[{ key, uid }]`, one per gallery collection you have
  // loaded, in load order. Your cloud profile stores THIS INSTEAD OF THEIR SCENES (cloudBlob):
  // somebody else's work is theirs to publish, so a copy in your document would be counted on
  // your gallery card and handed on again to anyone who loads you. What you did is that you
  // added their collection; that is the fact worth remembering, and the scenes themselves stay
  // in this browser.
  //
  // Keyed by the collection LABEL, which is what every scene carries and what dropCollection
  // works in; the uid rides along because it is the only durable handle on the source profile
  // (a profile can be renamed). Loading the same person twice refreshes rather than duplicates.
  //
  // `collectionsHeld` itself is declared UP IN audio-tuning-data.js, beside fullSnapshot and
  // applyBlob — the same split cpuBlocked uses. `restore()` runs three slices above this one,
  // so a `let` here would be in the TDZ the moment a stored blob carried the field, which is
  // every reload after the first. The functions are declarations and hoist, so they are safe
  // to call from up there.
  function noteCollection(key, uid) {
    key = (key || "").trim();
    if (!key) return;
    collectionsHeld = collectionsHeld.filter(c => c.key !== key);
    collectionsHeld.push({ key, uid: String(uid || "") });
  }
  function forgetCollection(key) { collectionsHeld = collectionsHeld.filter(c => c.key !== key); }
  // Same shape as palUse/transUse: validated on the way in, because it rides in a blob a
  // stranger can author. Anything malformed is dropped rather than rejecting the whole load.
  function collectionsOk(v) {
    if (!Array.isArray(v)) return [];
    const out = [], seen = new Set();
    for (const c of v) {
      if (!c || typeof c !== "object") continue;
      const key = typeof c.key === "string" ? c.key.trim().slice(0, 40) : "";
      if (!key || seen.has(key)) continue;
      seen.add(key);
      out.push({ key, uid: typeof c.uid === "string" ? c.uid.slice(0, 64) : "" });
  ...
  // No exceptions any more: the top group used to be opened once per load (so a first-time
  // visitor saw the shipped scenes rather than one closed "Default scenes 4" row), but that
  // was rolled back by request — EVERY group starts folded, your own included. The count on
  // each heading is what says there is something inside.
  ...
  function dropCollection(key, label) {
    ...
    forgetCollection(key);            // you no longer hold it, so your profile should stop saying you do
  ...
  el("delpreset").addEventListener("click", () => {
    if (curPreset < 0 || curPreset >= presets.length) return;
    // ASK FIRST. Delete sits one button along from Rename in a row you click while working, a
    // scene is the only copy of everything you tuned into it, and there is no undo — the next
    // persist() writes the shorter list. The other destructive buttons (Delete profile, Remove
    // collection, Delete palette, Reset) have always asked; this one never did.
    //
    // Named, because "are you sure?" does not tell you WHICH scene is selected, and the whole
