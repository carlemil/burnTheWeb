  ...
  // ---- THE PER-LAYER SALT IS SWITCHED OFF, DELIBERATELY AND TEMPORARILY ----------------
  // A ghost has been reported since the salt landed: a second copy of the balls, the same
  // animation but far behind, appearing on reload or when layers are rearranged and fading
  // after a few seconds. It could not be reproduced here -- the ball geometry was logged
  // frame by frame and is single-valued, the salt was instrumented and never flips, and the
  // reporter's own scene was fetched from its share link and measured for 148 frames after a
  // settle (frames two apart differ 1.59x more than adjacent ones, which is ordinary motion;
  // a flip between two pictures reads below 1). So the only reproducer is the reporter's
  // machine, and this exists to let THEM answer the question a repro would have answered:
  //
  //   ghost gone with this off  -> the salt is the cause, and where to look is known
  //   ghost still there         -> today's glass work is cleared, and the search moves on
  //
  // The cost of having it off is nil for a scene with ONE glass layer, which is the reported
  // setup: the salt exists only to keep TWO glass layers from drawing the same balls in the
  // same places. Setting this back to true restores that, and gbBallAt is untouched, so the
  // maths and its probe stay exactly as they were.
  const GB_SALT_ON = false;
