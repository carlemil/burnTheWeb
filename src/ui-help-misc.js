  ...
  const HELP = {
    ...
    sliders: [
      ...
      { n: "Share one 3D world", w: "all", t: "Traces this layer's geometry TOGETHER with every other layer that has it ticked, as one scene, instead of drawing it on its own. Tick it on an Ocean and a Glass ball and the balls really sit in the water: the water is reflected and refracted through them, they are reflected back in it, each hides the other where it should, and there is a real waterline across them. Only the effects that can share a viewpoint offer it — the ones you fly INSIDE (Mandelbulb, Menger sponge, Doughnut) never can. Each layer keeps its own palette, so the water seen inside the ball is tinted like the ball; that is deliberate, and it is the same rule the rest of the app follows. The camera comes from the lowest layer that has joined — its Camera X/Y/Z, Zoom and Field of view now move the whole world." },
