  ...
  const HELP = {
    ...
    sliders: [
      ...
      { n: "Field of view", w: "all", t: "How wide a lens this layer is seen through. It works the way the Camera X/Y/Z sliders do — by moving where each pixel takes its colour from — so it costs nothing and applies to every shader effect: positive pushes the sample outward with the square of the radius, which is a fisheye that bows the middle out and drags the corners in; negative pulls it in, which is a telephoto that flattens the frame. 0 is the normal lens. Drift the two thumbs and the whole layer breathes in and out. It is per LAYER, so one layer can bulge while another stays flat. Not offered on the point effects (Sierpński, Trees, Boids and the rest) — those stamp points rather than sampling a coordinate, so there is nothing to bend." },
      ...
      { n: "Reflection", w: "ocean", t: "Ocean only — how strongly the water mirrors THE LAYER UNDERNEATH IT. Put the Ocean on top of another layer and that layer appears in the water, concentrated toward the horizon the way a real reflection is: water is a mirror at a glancing angle and nearly clear straight down. With nothing beneath it there is nothing to reflect and this does nothing. What comes back is brightness, so the reflection is tinted by the water's own palette." },
