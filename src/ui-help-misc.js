  ...
  const HELP = {
    ...
    sliders: [
      { n: "Scenes", w: "all", t: "A saved scene is a named full snapshot — the effect plus every setting. Pick one to load it; from then on any change you make is auto-saved straight back into it. New saves what is on screen as a fresh scene, selects it and turns auto-cycle off so it stays up; Rename renames the selected one; Delete removes it. The first entry, “— unsaved scene —”, is not a saved scene but a mode: while it is selected nothing is auto-saved, so you can tweak freely without altering anything you have stored. You also land there whenever what is on screen is not one of your saved scenes — after deleting one, when opening a shared link, or after switching to an effect you have no scene for. Your whole library can be kept against a Google account in the Cloud profile box, and published there for other people to load." },
