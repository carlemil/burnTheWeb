  ...
  const EFFECTS = [
    ...
    { id: "mandelbulb", name: "Mandelbulb", subtitle: "Mandelbulb · flight inside a 3D fractal",
      help: "The 3D Mandelbrot: z → z^p + c run in spherical coordinates and raymarched as a solid, seen from INSIDE — the camera flies the canyons between the lobes, threading whatever gap the fractal leaves it and never quite touching the surface. Power reshapes it continuously — 8 is the classic bulb, low values melt it toward a sphere, high values bristle it, and the fractions in between morph smoothly (drift Power's two thumbs apart and the bulb never stops reshaping around you). Detail adds fractal depth (and GPU cost), Orbit speed is how fast the flight winds through it, Glow lights the walls and puts a halo where a ray only just misses. The heaviest effect in the app — it wants a real GPU.",
    ...
    { id: "galaxy", name: "Galaxy", subtitle: "Galaxy · log-spiral star disc",
      ...
      params: ["gxarms", "gxtwist", "gxspin", "gxcore", "gxscatter", "points", "zoom", "camrx", "camry", "camrz", "palcycle", "palhold"],
    ...
    { id: "ocean", name: "Ocean", subtitle: "Ocean · Gerstner swell to the horizon",
      ...
      draw: dt => { const s = oceanSeed(dt);
        if (useGL) glShaderDraw("ocean", u => { gl.uniform1f(u.uTime, s.t); gl.uniform1f(u.uSwell, s.swell); gl.uniform1f(u.uChop, s.chop); gl.uniform1f(u.uFoam, s.foam); gl.uniform1f(u.uWind, s.wind); gl.uniform1f(u.uZoom, s.zoom); });
        else oceanCPU(s); },
      defaults: { palcycle: [0, 0], palhold: [0, 0], goswell: [1, 1], gochop: [2.5, 2.5], gospeed: [1, 1], gofoam: [0.45, 0.45], gowind: [0, 0],
        zoom: [1, 1], band: [0, 0], bandsize: [1, 1], banddim: [0, 0] },
      beat: {}, extras: { palette: "1", morph: false, showBox: true, randSeed: true } },
