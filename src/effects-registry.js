  ...
  const EFFECTS = [
    ...
    { id: "lightning", name: "Lightning storm", subtitle: "Lightning · beat-fired bolts",
      help: "Bolts of lightning tearing down the screen, each one a fresh shape. Rate fires them on a clock; arm Strike's L/M/H chips and the BEAT fires them instead — a kick snaps Strike to full and the bolt decays over the Trigger duration, exactly like Shockwave's ring. Bolts sets how many strike at once, Afterglow how much the sky lights up.",
      params: ["ltstrike", "ltrate", "ltbolts", "ltglow", "zoom", "camrx", "camry", "camrz", "palcycle", "palhold", "band", "bandsize", "banddim"], helpTags: ["all", "storm", "band"], bakesOwnZoom: true,
      draw: dt => { if (useGL) { const s = stormSeed(dt); glShaderDraw("storm", u => { gl.uniform1f(u.uEnv, s.env); gl.uniform1f(u.uSeed, s.seed); gl.uniform1f(u.uBolts, s.bolts); gl.uniform1f(u.uGlow, s.glow); gl.uniform1f(u.uZoom, s.zoom); }); } else storm(dt); },
      defaults: { palcycle: [0, 0], palhold: [0, 0], ltstrike: [0, 0], ltrate: [0.5, 0.5], ltbolts: [2, 2], ltglow: [0.5, 0.5], zoom: [1, 1], band: [0, 0], bandsize: [1, 1], banddim: [0, 0] },
      beat: {}, extras: { palette: "7", morph: false, showBox: true, randSeed: true } },
    { id: "mandelbulb", name: "Mandelbulb", subtitle: "Mandelbulb · raymarched 3D fractal",
      help: "The 3D Mandelbrot: z → z^p + c run in spherical coordinates and raymarched as a solid, slowly orbited by the camera. Power reshapes it — 8 is the classic bulb, low values melt it toward a sphere, high values bristle it. Detail adds fractal depth (and GPU cost), Orbit speed sets the camera's lap, Glow lights the silhouette and puts a halo on near-misses. The heaviest effect in the app — it wants a real GPU.",
    ...
    { id: "reactdiff", name: "Reaction-diffusion", subtitle: "Gray–Scott · living patterns",
      help: "Two chemicals feeding and killing each other on a dish, forming spots, stripes, coral and mazes that grow and never repeat. Feed and Kill choose the regime — tiny nudges cross into whole new pattern families, so arm Feed's chips and the beat pushes the culture into a new life. Sim speed is how many chemistry steps run per frame; the culture re-seeds every time you switch to the effect. The state is ONE shared dish: two layers of it show the same culture.",
      ...
      defaults: { palcycle: [0, 0], palhold: [0, 0], rdfeed: [0.037, 0.037], rdkill: [0.065, 0.065], rdspeed: [8, 8], rdgain: [1, 1], zoom: [1, 1], band: [0, 0], bandsize: [1, 1], banddim: [0, 0] },
      beat: {}, extras: { palette: "2", morph: false, showBox: true, randSeed: true } },
    { id: "menger", name: "Menger sponge", subtitle: "Menger · infinite lattice dive",
      help: "An endless lattice of Menger sponges — the 3D Sierpiński carpet — raymarched while the camera dives forward through the holes with a slow roll. Dive speed is the flight, Detail the fractal depth (and the GPU bill), Glow lights the tunnel. Kin to Sierpiński: this is what its carpet looks like grown into a solid.",
      ...
      draw: dt => { if (useGL) { const s = mengerSeed(dt); glShaderDraw("menger", u => { gl.uniform1f(u.uDive, s.z); gl.uniform1f(u.uSpin, s.spin); gl.uniform1f(u.uIter, s.iter); gl.uniform1f(u.uGlow, s.glow); gl.uniform1f(u.uZoom, s.zoom); }); } else menger(dt); },
