  ...
  function paleApply(full, dragLive) {
    ...
    // Mid-drag, refresh the gradient and fields but DO NOT rebuild the handle buttons —
    // the drag's pointer capture and listeners live on one of them (see paleDragStart).
    if (dragLive) {
      el("pale-grad").style.background = paleGradCss(st);
      if (paleSel) el("pale-pos").value = Math.round(paleSel[0] * 100);
    } else paleRender();
  }
  ...
  //
  // NOTHING in this gesture may rebuild #pale-stops: the pointer capture and the move/up
  // listeners live on `btn`, and a rebuild removes it from the document — which drops the
  // capture (the same Chromium rule as the filter-list drag) and the gesture goes dead on
  // its first frame. That is exactly the bug that shipped: paleDragStart ended in
  // paleRender(), so the handles LOOKED draggable but only ever registered the click.
  // Selection is painted in place, the moves go through paleApply's dragLive path, and the
  // full re-render happens once on release.
  function paleDragStart(ev, btn, stop) {
    ev.preventDefault(); ev.stopPropagation();
    paleSel = stop;
    const bar = el("pale-grad");
    for (const h of el("pale-stops").children) h.classList.toggle("sel", h === btn);
    el("pale-color").value = paleHex(stop[1]);
    el("pale-pos").value = Math.round(stop[0] * 100);
    // Synthetic probe events carry no live pointer, and capture is a nicety (the listeners
    // sit on the button either way) — so a capture failure must not abort the drag.
    try { btn.setPointerCapture(ev.pointerId); } catch (e) {}
    const move = e => {
      const r = bar.getBoundingClientRect();
      stop[0] = Math.max(0, Math.min(1, (e.clientX - r.left) / (r.width || 1)));
      paleStops().sort((a, b) => a[0] - b[0]);
      btn.style.left = (stop[0] * 100) + "%";
      btn.title = Math.round(stop[0] * 100) + "% · " + paleHex(stop[1]);
      paleDirty = true;
      paleApply(false, true);       // gradient + fields only; the handle nodes stay alive
    };
    const up = () => {
      ...
      paleRender();                 // the full rebuild is safe now the gesture is over
    };
