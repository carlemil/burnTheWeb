  ...
  const HELP = {
    ...
    sliders: [
      ...
      { n: "Drift speed", w: "fire", t: "Sierpiński: how fast the triangle's three corners wander. Tetrafyer: the physics tempo (how fast the tetrahedron bounces)." },
      { n: "Size", w: "fire", t: "Scales the fractal itself about its centre — Sierpiński's triangle or Tetrafyer's tetrahedron (which grows/shrinks with matching physics). Separate from Zoom, which scales the whole view." },
      { n: "Rotation", w: "tetra", t: "Tetrafyer only — the yaw rate in degrees per second for the scene orbit around the box. It ships drifting between −5 and 5, so the box is never quite still; pinch both thumbs to 0 to stop it." },
      { n: "Box nod", w: "tetra", t: "Tetrafyer only — how far the view pitches up and down in its slow sine, in degrees. This is the second, slower drift that used to be built in with no control at all; 0 holds the box dead level." },
      { n: "Nod speed", w: "tetra", t: "Tetrafyer only — how fast that nod swings, as a multiplier; 0 freezes it mid-swing. Drift speed scales it too, so a faster scene nods faster." },
      { n: "Zoom", w: "all", t: "Scales the whole view in and out." },
      { n: "Camera X / Y / Z", w: "all", t: "Tilt and spin the whole scene in 3D, in degrees. X and Y rock it away from you; Z rolls it in the plane of the screen. These are global — they stay put as you switch effects and are not part of any one effect's settings — so they act as a camera over whatever is running. The startup credits deliberately ignore them and stay level." },
      { n: "Filters", w: "all", t: "A stack of post-effects any visual can use, ticked in the Filters section; they apply in the order listed. Fire and Fade pixel feed back into the picture, building up over successive frames; the rest are passes over the finished image. Ticking one reveals its own sliders. Without WebGL the GPU-only ones are greyed out." },
      ...
      { n: "Bloom", w: "all", t: "The additive glow around the brightest areas. Strength 0 turns it off entirely — this is the glow that used to be permanent." },
