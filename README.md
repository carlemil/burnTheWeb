...
| **Effect** | Switch between all twenty effects listed above, in dropdown order (Sierpiński, Tetrahedron, AnimeJulia, Plasma, Tunnel, Metaballs, Burning Ship, Kaleidoscope, Rotozoomer, Munching Squares, Moiré, Newton, Multibrot, Copper Bars, Attractor, Polygon, Shape grid, Concentric rings, Bouncing shapes, Bouncing solids). Each shows its own sliders. There are **two** copies of this chooser and they do the same thing: one on every row in the **Layers** box (so you can re-point a layer without leaving the list — picking on a row that isn't selected selects it first), and one at the top of the **Effect & Filters** box for the layer you have selected. |
