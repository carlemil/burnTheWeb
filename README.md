...
| **Collections** | The scene list is grouped, and each group folds. The first is **yours** (labelled with your cloud profile name once you have one, otherwise "My scenes"); every other one is somebody else's, named after them, created when you load their scenes from the gallery. Their scenes never mix with yours: their "Sunset" and your "Sunset" sit in different collections and neither overwrites the other, and loading the same person again refreshes just their set. **Collections start folded**, so the list opens as a short stack of names — click a name to open it, click again to fold it. **✕** on a collection removes that whole set (your own group has no ✕, because it isn't something you loaded). |
