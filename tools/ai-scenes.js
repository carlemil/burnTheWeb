...
if (argv[0] === "--emit") {
  // The BUILT-IN gallery row: the same deflated blob the #zp= link carries, shipped inside the
  // app so the Public scenes dialog lists these scenes without an account or a network.
  const payload = zlib.deflateRawSync(Buffer.from(JSON.stringify(blob))).toString("base64url");
  const src = [
    "  // ---- built-in gallery entries ----------------------------------------------------",
    "  // GENERATED by 'node tools/ai-scenes.js --emit' -- do not hand-edit. Rows the Public scenes",
    "  // dialog lists WITHOUT any network, ahead of the published profiles. 'payload' is the same",
    "  // deflated libraryUrl-shaped blob a cloud profile carries, so galFetchLibrary decodes it",
    "  // through the one path and a follow of it (uid 'builtin:...') resolves offline too.",
    "  const BUILTIN_GALLERY = [",
    "    { uid: \"builtin:ai-generated\", name: \"AI-Generated\", count: " + SCENES.length
      + ", updated: \"" + new Date().toISOString().slice(0, 10) + "\",",
    "      payload: \"" + payload + "\" },",
    "  ];",
    "",
  ].join("\n");
  const out = path.join(__dirname, "..", "src", "gallery-builtin.js");
  fs.writeFileSync(out, src);
  console.log("wrote " + out + " (" + SCENES.length + " scenes, " + payload.length + " chars)");
} else if (argv[0] === "--json") {
