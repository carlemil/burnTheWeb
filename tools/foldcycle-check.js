...
const ASSERT_JS = [
  ...
  "    // THE PALETTE CHEVRON IS THE LAYER CHEVRON. Asserted on computed style, not a",
  "    // screenshot: the first version sat inside the <label> and picked up `#panel .lyr b`",
  "    // button chrome (10px font, 1px border, dark fill) and read as a little boxed button.",
  "    // Every layer starts FOLDED (the palette block is display:none inside it), so unfold",
  "    // the first row before looking for a visible palette chevron.",
  "    var lc = document.querySelector(\"#panel .lyr b.lyr-chev\");",
  "    if (lc) lc.click();",
  "    var pc = [].slice.call(document.querySelectorAll(\"#panel .pal-chev\")).filter(function(n){ return n.offsetParent !== null; })[0];",
  "    if (lc && pc) {",
  "      var LC = getComputedStyle(lc), PC = getComputedStyle(pc);",
  "      var same = [\"width\",\"height\",\"fontSize\",\"color\",\"opacity\",\"borderTopWidth\",\"backgroundColor\",\"paddingLeft\"]",
  "        .filter(function(k){ return LC[k] !== PC[k]; });",
  "      ok(\"the palette chevron computes identically to the layer chevron\", same.length === 0, same.join(\",\") || \"8 props\");",
  "      ok(\"...and is not inside the label\", !pc.closest(\"label\"));",
  "      // Banding is a filter over the palette, so it folds with it.",
  "      var bc = pc.closest(\".lyrblock\").querySelector(\"[data-k=bandctl]\");",
  "      ok(\"the banding controls sit inside the palette fold body\", !!bc && !!bc.closest(\"[data-k=palbody]\"));",
  "    } else ok(\"both chevrons are on screen\", false);",
  "",
